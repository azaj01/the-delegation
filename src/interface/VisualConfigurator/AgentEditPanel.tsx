...
import { X, Save, Trash2, Shield, Target, Cpu } from 'lucide-react';
...
export const AgentEditPanel: React.FC<AgentEditPanelProps> = ({ agent, onClose, onSave }) => {
  return (
    <div className="w-80 h-full bg-white border-l border-zinc-200 flex flex-col shadow-2xl pointer-events-auto overflow-hidden animate-in slide-in-from-right-full duration-300">
      {/* Header */}
      <div className="p-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: agent.color }} />
          <h3 className="font-bold text-sm text-zinc-800 uppercase tracking-tight">Configure Agent</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <section className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 block">Description (Optional)</label>
          </div>
          <input
            type="text"
